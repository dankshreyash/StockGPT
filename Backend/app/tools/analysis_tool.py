...
def generate_general_answer(context_data: dict, user_question: str, history: list = None) -> str:
    system_msg = "You are a helpful financial assistant. Use the conversation history to understand context and pronouns like 'it' or 'this'."
    ...
    prompt = f"""Answer the following financial question clearly and concisely. Use conversation history for context.
...
User Question: {user_question}"""
    ...
    for attempt in range(3):
        try:
            ...
            return strip_thinking_tags(chat_completion.choices[0].message.content)
        except Exception as e:
