...
const StockCard = ({ data }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 my-2 max-w-sm w-full relative overflow-hidden group"
    >
      ...
      <div className="flex justify-between items-start mb-4 relative z-10">
        <div className="max-w-[65%]">
          ...
          <p className="text-sm text-zinc-400 truncate" title={data.company}>{data.company}</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4 relative z-10">
        <div className="bg-zinc-950/50 p-2 rounded-xl border border-white/5">
          <p className="text-xs text-zinc-500 mb-1">Market Cap</p>
          <p className="text-sm font-semibold text-zinc-200">{data.market_cap_display || 'N/A'}</p>
        </div>
        <div className="bg-zinc-950/50 p-2 rounded-xl border border-white/5">
          <p className="text-xs text-zinc-500 mb-1">P/E Ratio</p>
          <p className="text-sm font-semibold text-zinc-200">{data.pe ? data.pe.toFixed(2) : 'N/A'}</p>
        </div>
        <div className="bg-zinc-950/50 p-2 rounded-xl border border-white/5">
          <p className="text-xs text-zinc-500 mb-1">52W High</p>
          <p className="text-sm font-semibold text-zinc-200">₹{data.high52 ? data.high52.toFixed(2) : 'N/A'}</p>
        </div>
        <div className="bg-zinc-950/50 p-2 rounded-xl border border-white/5">
          <p className="text-xs text-zinc-500 mb-1">52W Low</p>
          <p className="text-sm font-semibold text-zinc-200">₹{data.low52 ? data.low52.toFixed(2) : 'N/A'}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2 pt-3 border-t border-white/10 relative z-10">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="text-zinc-300 font-medium text-right">{data.sector || 'N/A'}</span>
        </div>
        <div className="flex items-center justify-between text-sm">
          <span className="text-zinc-500">Industry</span>
          <span className="text-zinc-300 font-medium text-right">{data.industry || 'N/A'}</span>
        </div>
        {data.website && (
