...
const TechnicalCard = ({ data }) => {
  ...
  const MetricBox = ({ label, value }) => (
    <div className="bg-zinc-950/50 p-2 rounded-lg border border-white/5 flex flex-col justify-center items-center">
      <span className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1">{label}</span>
      <span className="text-sm font-semibold text-zinc-200">{value ?? 'N/A'}</span>
    </div>
  );
    ...
    >
      <div className="flex items-center justify-between mb-4 border-b border-white/10 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-white tracking-tight">Technical Analysis</h3>
        </div>
        <div className="flex items-center gap-1.5 bg-zinc-950 px-2.5 py-1 rounded-full border border-white/5">
          {getTrendIcon()}
          <span className="text-xs font-semibold text-zinc-300">{data.trend}</span>
        </div>
      </div>
