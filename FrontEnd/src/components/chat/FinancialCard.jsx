...
const FinancialCard = ({ data }) => {
  ...
  const MetricBox = ({ label, value, highlight = false }) => (
    <div className={`p-2.5 rounded-xl border ${highlight ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-zinc-950/50 border-white/5'} flex flex-col justify-center`}>
      <span className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1 block truncate">{label}</span>
      <span className={`text-sm font-semibold truncate ${highlight ? 'text-emerald-400' : 'text-zinc-200'}`}>{value ?? 'N/A'}</span>
    </div>
  );
    ...
    >
      <div className="flex items-center justify-between mb-4 border-b border-white/10 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-white tracking-tight">Financial Health</h3>
        </div>
        {data.quarter_ended && (
          <span className="text-[10px] text-zinc-500 bg-zinc-950 px-2 py-1 rounded-md border border-white/5">
