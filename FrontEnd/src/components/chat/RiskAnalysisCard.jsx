  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-4 border-b border-white/10 pb-3">
        ...
        <h3 className="text-sm font-bold text-white tracking-tight">Risk Analysis</h3>
      </div>

      <div className="grid grid-cols-2 gap-2">
        {data.risk_analysis.map((risk, index) => (
          <div key={index} className="bg-zinc-950 p-2.5 rounded-xl border border-white/5 flex flex-col justify-between">
            <div className="flex items-start gap-1.5 mb-2">
              <FiAlertOctagon className="text-zinc-500 mt-0.5 shrink-0" size={12} />
              <span className="text-xs text-zinc-300 font-medium leading-tight">{risk.name}</span>
            </div>
