  ...
  const MetricRow = ({ label, valA, valB, isBetterA = null }) => {
    return (
      <div className="flex items-center justify-between py-2 border-b border-white/5 text-sm">
        <div className="w-1/3 font-semibold text-zinc-200 text-left truncate pr-2">{valA || 'N/A'}</div>
        <div className="w-1/3 text-center text-zinc-500 text-xs uppercase tracking-wider">{label}</div>
        <div className="w-1/3 font-semibold text-zinc-200 text-right truncate pl-2">{valB || 'N/A'}</div>
      </div>
    );
  };
    ...
    >
      <div className="flex items-center justify-between mb-4 border-b border-white/10 pb-3">
        <div className="text-left w-2/5">
          <h3 className="text-lg font-bold text-white tracking-tight truncate" title={stockA.symbol}>{stockA.symbol}</h3>
          <p className="text-xs text-zinc-400 truncate" title={stockA.company}>{stockA.company}</p>
        </div>
        <div className="text-center w-1/5 text-zinc-600 font-bold italic">VS</div>
        <div className="text-right w-2/5">
          <h3 className="text-lg font-bold text-white tracking-tight truncate" title={stockB.symbol}>{stockB.symbol}</h3>
          <p className="text-xs text-zinc-400 truncate" title={stockB.company}>{stockB.company}</p>
        </div>
      </div>
